...
| 注释类型 | 是否保留原文 | 说明 |
|---------|------------|------|
| 类注释 | 视情况 | 简单类注释可不保留；复杂或难懂的类必须保留 |
| 枚举注释 | 视情况 | 简单枚举值可不保留；含义复杂的必须保留 |
| 方法注释 | **必须** | 所有方法注释都必须保留原文 |
| 字段注释 | **必须** | 所有字段注释都必须保留原文 |
| 构造方法注释 | **必须** | 所有构造方法注释都必须保留原文 |
| 接口/抽象类注释 | **必须** | 必须保留原文 |
...
- "原文：" 三个字后面紧跟英文原文，**不加空行**
- 原文段落前使用 `<p>` 标签换行
- 原文与译文必须是连贯的，**禁止**译文一句原文一句交替排列

### 何时添加译注
当遇到以下情况时，应在"原文："之前添加译注：
- 术语在中文中没有直接对应表达
- 原文含义模糊或容易引起歧义
- 需要补充 Minecraft 游戏内的具体机制说明
- 翻译后难以理解的技术概念
...
请**统一使用英文标点符号**，禁止混用中文标点：

| 中文标点 | 英文标点 | 使用场景 |
|---------|---------|---------|
| `。` | `. ` | 句末结束（句号后必须加空格，JavaDoc 使用 ". " 区分简短描述和详细描述） |
| `，` | `, ` | 句中停顿 |
| `：` | `: ` | 冒号后说明 |
| `；` | `; ` | 分号分隔并列内容 |
| `（）` | `()` | 括号内补充说明 |
...
**特别注意**：
- 简述（第一段）必须以 `. `（英文句号加空格）结尾
- 详细描述中也统一使用英文标点
- 参数说明 `@param`、返回值 `@return`、异常 `@throws` 后的描述同样使用英文标点

## 格式规范

### 换行规则
- **必须在需要换行的地方使用 `<p>`**，否则 JavaDoc 不会换行
- 简述与详细描述之间：使用 `<p>` 分隔
- 详细描述与"原文："之间：使用 `<p>` 分隔
- 原文与标签（@param/@return 等）之间：使用空行分隔

### 缩进规则
- 多行注释的 `*` 号应对齐，保持一致的缩进
- 避免在 `{@link ...}` 前添加多余空格造成格式不美观

### 标签规范
- `@param` 参数名**不翻译**，仅翻译参数说明
- `@return` 描述使用英文标点
- `@throws` 说明通常以"if"或"如果"开头，保留异常类名
- `@see` 不必翻译，保留原格式
- `{@link 类名#成员名}` 保留原格式，有 import 时省略包名

## 术语一致性要求

同一术语在同一项目中的所有文件中必须保持统一翻译：

| 英文术语 | 推荐译名 | 禁止使用的其他译名 |
|---------|---------|------------------|
| hover event | 悬停事件 | 悬浮事件 |
| click event | 点击事件 | 单击事件 |
| obfuscated | 模糊(随机字符) | 随机字符（单独使用时不完整） |
| legacy | 旧版/传统 | 遗留 |
| format | 格式 | 格式化 |

### 术语查询流程
1. 遇到专有名词时，首先在 `./data` 目录下用 `grep` 搜索
2. 优先使用 CSV 中的标准化映射
3. 搜索不到时，参考官方 Minecraft Wiki 中文版
4. 仍不确定时，保留原文并添加译注
...
## 批量修改限制
- 不要一次性编辑过多的行，否则可能会出现工具异常
- 一般每次只修改 **50 行**，最多不建议超过 **100 行**
- 读取文件时可以直接读取全部内容

## 翻译范围限制
- **只翻译 JavaDoc 注释**，严禁修改代码逻辑、方法签名、类结构
- 严禁添加、删除、重命名方法或字段
- 严禁修改 import 语句（除非是为了修复已有的错误 import）
- 如果原文件有代码结构问题，应报告给用户而不是自行修改

## 修改前检查清单
在提交修改前，请确认：
- [ ] 所有方法注释和字段注释都保留了"原文："段落
- [ ] 所有标点符号均为英文标点
- [ ] 简述以 `. ` 结尾
- [ ] 需要换行的地方使用了 `<p>`
- [ ] 专有名词使用了统一译名
